...
class VideoFrameExtractor:
    ...
    def setup_ui(self):
        ...
        video_frame = ttk.LabelFrame(
            main_frame, text="Selección de Video", padding="10")
        video_frame.grid(
            row=0,
            column=0,
            columnspan=3,
        ...
        self.video_label = ttk.Label(
            video_frame, text="No se ha seleccionado ningún video")
        self.video_label.grid(row=0, column=0, sticky=tk.W, padx=5)

        ttk.Button(
            video_frame,
            text="Seleccionar Video",
            command=self.select_video).grid(
            row=0,
            column=1,
            padx=5)
        ...
        frame_frame = ttk.LabelFrame(
            main_frame,
            text="Intervalo de Fotogramas",
            padding="10")
        frame_frame.grid(
            row=3,
            column=0,
        ...
        ttk.Label(
            frame_frame,
            text="Extraer cada N fotogramas:").grid(
            row=0,
            column=0,
            sticky=tk.W,
            padx=5)
        ...
        ttk.Label(
            frame_frame,
            text="(ej: 30 = un fotograma cada 30 fotogramas)").grid(
            row=0,
            column=2,
            sticky=tk.W,
            padx=5)
        ...
        ttk.Button(
            gps_frame,
            text="Cargar Datos GPS (JSON)",
            command=self.load_gps_data).grid(
            row=0,
            column=0,
            padx=5)

        self.gps_status_label = ttk.Label(
            gps_frame, text="No se han cargado datos GPS")
        self.gps_status_label.grid(row=0, column=1, sticky=tk.W, padx=5)

        # Carpeta de salida
        output_frame = ttk.LabelFrame(
            main_frame, text="Carpeta de Salida", padding="10")
        output_frame.grid(
            row=5,
            column=0,
            columnspan=3,
        ...
        self.output_label = ttk.Label(
            output_frame, text="No se ha seleccionado carpeta de salida")
        self.output_label.grid(row=0, column=0, sticky=tk.W, padx=5)

        ttk.Button(
            output_frame,
            text="Seleccionar Carpeta",
            command=self.select_output).grid(
            row=0,
            column=1,
            padx=5)
        ...
        self.progress.grid(
            row=7, column=0, columnspan=3, sticky=(
                tk.W, tk.E), pady=5)
    ...
    def load_video_info(self):
        ...
        try:
            ...
            self.video_duration = self.video_total_frames / \
                self.video_fps if self.video_fps > 0 else 0
            cap.release()

            info_text = f"FPS: {
                self.video_fps:.2f} | Duración: {
                self.video_duration:.2f}s | Fotogramas totales: {
            ...
            messagebox.showerror(
                "Error",
                f"Error al cargar información del video: {
                    str(e)}")
    ...
    def load_gps_data(self):
        ...
        if filename:
            try:
                ...
                if not isinstance(
                        gps_data,
                        dict) or 'latitude' not in gps_data or \
                        'longitude' not in gps_data:
                    messagebox.showerror(
                        "Error",
                        "El archivo JSON de GPS debe contener los campos "
                ...
                messagebox.showerror("Error",
                                     f"Error al cargar datos GPS: {str(e)}")
    ...
    def add_gps_to_image(self, image_path, lat, lon, alt=None):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"Error inesperado al cargar EXIF de {image_path}: {
                        str(e)}")
                exif_dict = {
                    "0th": {},
                    "Exif": {},
            ...
            messagebox.showerror(
                "Error",
                f"Error al agregar GPS a {image_path}: {
                    str(e)}")
    ...
    def extract_frames(self):
        ...
        if not self.video_path:
            messagebox.showwarning(
                "Advertencia", "Por favor, seleccione un video")
            return
        ...
        try:
            ...
            if interval < 1:
                messagebox.showerror(
                    "Error", "El intervalo de fotogramas debe ser al menos 1")
                return
        ...
        try:
            ...
            while True:
                ...
                if (current_frame - start_frame) % interval == 0:
                    ...
                    self.status_label.config(
                        text=f"Extrayendo: {extracted_count}/"
                             f"{total_to_extract} fotogramas"
                    )
                    self.root.update()
            ...
            self.status_label.config(
                text=f"¡Extracción completada! {extracted_count} "
                     f"fotogramas guardados"
            )
            messagebox.showinfo(
                "Éxito",
